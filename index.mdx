---
hidden: true
mode: "frame"
---

export const HeroCard = ({ image, title, description, href }) => {
  return (
    <a
      className="link group cursor-pointer pb-8"
      href={href}
    >
      <img src={image} alt={title} className="pointer-events-none" />
      <h3 className="text-xl mt-5 text-gray-900 dark:text-zinc-50 font-medium">
        {title}
      </h3>
      <p className="block mt-1.5">
        {description}
      </p>
    </a>
  );
};

<div className="relative overflow-hidden">
  {/* Hero background */}
  <div className="absolute -top-14 left-0 right-0 opacity-80 pointer-events-none">
    <img
      src="https://mintcdn.com/mintlify/aol5WIB48xAgY4dd/images/hero/background-light.svg?w=840&fit=max&auto=format&n=aol5WIB48xAgY4dd&q=85&s=15a026af4a3362c67978a6d322f8ab5d"
      className="block w-full h-auto"
      alt="Image"
    />
  </div>

  {/* Hero content */}
  <div className="relative z-10 px-4 pt-16 lg:pt-40  max-w-3xl mx-auto">
    <h1 className="block text-4xl font-medium text-center text-gray-900 dark:text-zinc-50 tracking-tight">
     Squeditor's Documentation
    </h1>

    <p className="max-w-xl mx-auto px-4 mt-4 text-lg text-center text-gray-500 dark:text-zinc-500">
      A modern visual editor and a static HTML website framework for static site templates developers. Edit locally, customize freely, and ship beautiful sites.
    </p>

    <div className="px-6 lg:px-0 mt-12 lg:mt-16 grid sm:grid-cols-2 gap-8">
      <HeroCard
        image="/images/the-editor.png"
        title="The Editor Docs"
        description="A powerful no-code HTML editor built for teams, startups and agencies"
        href="/editor/getting-started/introduction"
      />
      <HeroCard
        image="/images/the-framework.png"
        title="The Framework Docs"
        description="Build with PHP templating, Tailwind, GSAP animations, and more."
        href="/framework/getting-started/introduction"
      />
    </div>
  </div>

  <div className="relative z-10 px-4 py-16 lg:pb-24 max-w-3xl mx-auto">
    <h2 className="block text-2xl font-medium text-center text-gray-900 dark:text-zinc-50 tracking-tight mb-8">
     Our templates core features
    </h2>
    <CardGroup cols={3}>
      <Card title="Zero-Runtime PHP Templating" icon="square-dashed-bottom-code">
      </Card>
      <Card title="Token-Based Styling Architecture" icon="swatch-book">
      </Card>
      <Card title="TailwindCSS v3 Foundation" icon="container">
      </Card>
      <Card title="Selective UIKit 3 Integration" icon="bolt">
      </Card>
      <Card title="Smooth Animations & Interactions" icon="blocks">
      </Card>
      <Card title="Well Organized & Documented" icon="file-code">
      </Card>
      <Card title="Multi-Theme Schema Support" icon="shapes">
      </Card>
      <Card title="Built-in Light/Dark Theme Mode" icon="moon">
      </Card>
      <Card title="Section-Based Workflow" icon="layers-2">
      </Card>
    </CardGroup>
  </div>
</div>